...
class BaseTestCase(SnippetTestBase):
    def test_snippet_manual(self):
        ...
        self.assertEqual(failed_span.get("status", {}).get("code"), "STATUS_CODE_ERROR")
        self.assertIn("Payment error:", failed_span.get("status", {}).get("message", ""))

        # Check for exception events with additional attributes
        events = failed_span.get("events", [])
        exception_events = [event for event in events if event.get("name") == "exception"]
        self.assertGreaterEqual(len(exception_events), 1, "Should have at least one exception event")
        ...
        self.assertEqual("ConnectionError", exception_attributes.get("error.type"))
        self.assertEqual("payment processing", exception_attributes.get("error.stage"))
